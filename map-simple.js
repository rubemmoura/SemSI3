function initMap() {
    ...
    if(sessionStorage.getItem('nomeprojeto')){
        ...
        console.log(sessionStorage.getItem('nomeprojeto'));
        var contentStringLocal = '<div id="content">'+
            '<div id="siteNotice">'+
            '</div>'+
            '<h1 id="firstHeading" class="firstHeading">'+ sessionStorage.getItem('nomeprojeto') + '</h1>'+
            '<div id="bodyContent" style=" text-align:justify; text-justify:inter-word;">'+
            '<p>'+ sessionStorage.getItem('oquefaz') + '</p>'+
            '<p>Telefone: '+ sessionStorage.getItem('telefone') + '</p>'+ '<img src="' + sessionStorage.getItem('foto') + '" style="width:100%;height:50%;">'+
            '</div>'+
            '</div>';

        var infoWindowMarkerLocal = new google.maps.InfoWindow({
            content: contentStringLocal,
            maxWidth: 300
        });

        var positionMarkerLocal = {lat:parseFloat(sessionStorage.getItem('latitude')), lng:parseFloat(sessionStorage.getItem('longitude'))};

        var markerLocal = new google.maps.Marker({
            position: positionMarkerLocal,
            map: map,
            title: sessionStorage.getItem('nomeprojeto'),
            animation: google.maps.Animation.DROP
        });

        markerLocal.addListener('click', toggleBounceLocal);


        function toggleBounceLocal() {
            if (markerLocal.getAnimation() !== null) {
                markerLocal.setAnimation(null);
            } else {
                markerLocal.setAnimation(google.maps.Animation.BOUNCE);
                infoWindowMarkerLocal.open(map, markerLocal);
                setTimeout(function(){
                    markerLocal.setAnimation(null); 
                }, 2200);
            }
    ...
    if(sessionStorage.getItem('nomeprojeto2')){
        ...
        console.log(sessionStorage.getItem('nomeprojeto2'));
        var contentStringLocal = '<div id="content">'+
            '<div id="siteNotice">'+
            '</div>'+
            '<h1 id="firstHeading" class="firstHeading">'+ sessionStorage.getItem('nomeprojeto2') + '</h1>'+
            '<div id="bodyContent" style=" text-align:justify; text-justify:inter-word;">'+
            '<p>'+ sessionStorage.getItem('oquefaz2') + '</p>'+
            '<p>Telefone: '+ sessionStorage.getItem('telefone2') + '</p>'+ '<img src="' + sessionStorage.getItem('foto2') + '" style="width:100%;height:50%;">'+
            '</div>'+
            '</div>';

        var infoWindowMarkerLocal2 = new google.maps.InfoWindow({
            content: contentStringLocal,
            maxWidth: 300
        });

        var positionMarkerLocal = {lat:parseFloat(sessionStorage.getItem('latitude2')), lng:parseFloat(sessionStorage.getItem('longitude2'))};

        var markerLocal2 = new google.maps.Marker({
            position: positionMarkerLocal,
            map: map,
            title: sessionStorage.getItem('nomeprojeto2'),
            animation: google.maps.Animation.DROP
        });

        markerLocal2.addListener('click', toggleBounceLocal2);


        function toggleBounceLocal2() {
            if (markerLocal2.getAnimation() !== null) {
                markerLocal2.setAnimation(null);
            } else {
                markerLocal2.setAnimation(google.maps.Animation.BOUNCE);
                infoWindowMarkerLocal2.open(map, markerLocal2);
                setTimeout(function(){
                    markerLocal2.setAnimation(null); 
                }, 2200);
            }
    ...
    if(sessionStorage.getItem('nomeprojeto3')){
        ...
        console.log(sessionStorage.getItem('nomeprojeto3'));
        var contentStringLocal = '<div id="content">'+
            '<div id="siteNotice">'+
            '</div>'+
            '<h1 id="firstHeading" class="firstHeading">'+ sessionStorage.getItem('nomeprojeto3') + '</h1>'+
            '<div id="bodyContent" style=" text-align:justify; text-justify:inter-word;">'+
            '<p>'+ sessionStorage.getItem('oquefaz3') + '</p>'+
            '<p>Telefone: '+ sessionStorage.getItem('telefone3') + '</p>'+ '<img src="' + sessionStorage.getItem('foto3') + '" style="width:100%;height:50%;">'+
            '</div>'+
            '</div>';

        var infoWindowMarkerLocal3 = new google.maps.InfoWindow({
            content: contentStringLocal,
            maxWidth: 300
        });

        var positionMarkerLocal = {lat:parseFloat(sessionStorage.getItem('latitude3')), lng:parseFloat(sessionStorage.getItem('longitude3'))};

        var markerLocal3 = new google.maps.Marker({
            position: positionMarkerLocal,
            map: map,
            title: sessionStorage.getItem('nomeprojeto3'),
            animation: google.maps.Animation.DROP
        });

        markerLocal3.addListener('click', toggleBounceLocal3);


        function toggleBounceLocal3() {
            if (markerLocal3.getAnimation() !== null) {
                markerLocal3.setAnimation(null);
            } else {
                markerLocal3.setAnimation(google.maps.Animation.BOUNCE);
                infoWindowMarkerLocal3.open(map, markerLocal3);
                setTimeout(function(){
                    markerLocal3.setAnimation(null); 
                }, 2200);
            }
